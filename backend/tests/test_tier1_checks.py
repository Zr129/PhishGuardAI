...
class TestBrandImpersonationCheck:

    def setup_method(self):
        from checks.tier1_checks import BrandImpersonationCheck
        self.check = BrandImpersonationCheck()

    def test_hard_block_with_password_field(self, make_request, make_refined):
        """Brand in domain + password field = hard BLOCK."""
        data    = make_request(has_password_field=True)
        refined = make_refined(registered_domain="paypal-secure-login.com")
    ...
    def test_warn_without_password_field(self, make_request, make_refined):
        """Brand in domain without password = score +2, no block."""
        data    = make_request(has_password_field=False)
        refined = make_refined(registered_domain="paypal-secure-login.com")
    ...
    def test_legitimate_domain_not_flagged(self, make_request, make_refined):
        """paypal.com itself should NOT be flagged — exact base match."""
        data    = make_request(has_password_field=True)
        refined = make_refined(registered_domain="paypal.com")

        result = self.check.run(data, refined)

        assert result.triggered is False

    def test_legitimate_country_variant_not_flagged(self, make_request, make_refined):
        """amazon.co.jp should NOT be flagged — base is 'amazon' exactly."""
        data    = make_request(has_password_field=True)
        refined = make_refined(registered_domain="amazon.co.jp")

        result = self.check.run(data, refined)

        assert result.triggered is False

    def test_brand_extra_domains_not_flagged(self, make_request, make_refined):
        """outlook.com is a Microsoft extra — should NOT be flagged."""
        data    = make_request(has_password_field=True)
        refined = make_refined(registered_domain="outlook.com")

        result = self.check.run(data, refined)

        assert result.triggered is False

    def test_multiple_brands_checked(self, make_request, make_refined):
        """Test a sample of different brands are all covered."""
        brands_to_test = [
            ("microsoft-login.com",  "microsoft"),
            ("amazon-secure.com",    "amazon"),
            ("netflix-account.com",  "netflix"),
            ("google-verify.com",    "google"),
            ("apple-id-verify.com",  "apple"),
        ]
        for domain, brand in brands_to_test:
            data    = make_request(has_password_field=True)
            refined = make_refined(registered_domain=domain)
            result  = self.check.run(data, refined)
            assert result.triggered is True, f"Expected {domain} to trigger for brand '{brand}'"
            assert result.is_block is True

    def test_unrelated_domain_not_flagged(self, make_request, make_refined):
        """A domain with no brand name should not be flagged."""
        data    = make_request(has_password_field=True)
        refined = make_refined(registered_domain="random-site-xyz.com")
